...
public class SynchronousConnection implements PersistentConnection.Delegate {
    ...
    public SynchronousConnection(String host) {
        ...
        int jvmProxyPort = Integer.parseInt(System.getProperty("http.proxyPort"));
        Socket socket = null;
        try {
            socket = new Socket(System.getProperty("http.proxyHost"), jvmProxyPort == -1 ? 9095 : jvmProxyPort);
        } catch (IOException e) {
            e.printStackTrace();
        }
        HostInfo hostInfo = new HostInfo(repoInfo.host, repoInfo.namespace, repoInfo.secure, socket);
        connection = new PersistentConnectionImpl(context.getConnectionContext(), hostInfo, this);
        this.connectSemaphore = new Semaphore(0);
    }

    public void connect() {
        context.getRunLoop().scheduleNow(connection::initialize);
        try {
