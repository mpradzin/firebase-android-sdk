...
public class Repo implements PersistentConnection.Delegate {
    ...
    Repo(RepoInfo repoInfo, Context ctx, FirebaseDatabase database) {
        ...
        scheduleNow(this::deferredInitialization);
    }
    ...
    private void deferredInitialization() {
        HostInfo hostInfo = new HostInfo(repoInfo.host, repoInfo.namespace, repoInfo.secure, database.getProxySocket());
    ...
    private void restoreWrites(PersistenceManager persistenceManager) {
        ...
        for (final UserWriteRecord write : writes) {
            RequestResultCallback onComplete = (optErrorCode, optErrorMessage) -> {
                DatabaseError error = fromErrorCode(optErrorCode, optErrorMessage);
    ...
    void callOnComplete(
        ...
        if (onComplete != null) {
            ...
            postEvent(() -> onComplete.onComplete(error, ref));
        }
    }
    ...
    private void warnIfWriteFailed(String writeType, Path path, DatabaseError error) {
        ...
        if (error != null
            ...
            operationLogger.warn(writeType + " at " + path.toString() + " failed: " + error);
        }
    }
    ...
    private void rerunTransactionQueue(List<TransactionData> queue, Path path) {
        ...
        for (final TransactionData transaction : queue) {
            ...
            if (abortTransaction) {
                ...
                this.scheduleNow(() -> removeEventCallback(
                        new ValueEventRegistration(
                                Repo.this,
                ...
                callbacks.add(() -> transaction.handler.onComplete(callbackError, false, snapshot));
            }
        }
    ...
    private void aggregateTransactionQueues(
        ...
        node.forEachChild(tree -> aggregateTransactionQueues(queue, tree));
    }
    ...
    private Path abortTransactions(Path path, final int reason) {
        ...
        transactionNode.forEachDescendant(
                tree -> abortTransactionsAtNode(tree, reason));
        ...
        if (queue != null) {
            ...
            for (int i = 0; i < queue.size(); ++i) {
                ...
                if (transaction.status == TransactionStatus.SENT_NEEDS_ABORT) {
                    ...
                    callbacks.add(() -> transaction.handler.onComplete(abortError, false, null));
                }
            }
