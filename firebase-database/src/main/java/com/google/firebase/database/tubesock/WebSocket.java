...
public class WebSocket {
    ...
    public WebSocket(ConnectionContext context, URI url, String protocol) {
        this(context, url, protocol, null, null);
    }
     ...
     */
    public WebSocket(
            ConnectionContext context, URI url, String protocol, Map<String, String> extraHeaders, @Nullable Socket proxySocket) {
        innerThread = getThreadFactory().newThread(() -> runReader(proxySocket));
        this.url = url;
    ...
    private void runReader(Socket proxySocket) {
        try {
            Socket socket = proxySocket != null ? proxySocket : createSocket();
            synchronized (this) {
