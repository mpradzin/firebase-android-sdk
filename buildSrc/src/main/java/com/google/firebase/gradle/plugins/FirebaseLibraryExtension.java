...
public class FirebaseLibraryExtension {
    ...
    private Action<MavenPom> customizePomAction = pom -> {
        ...
        pom.scm(scm -> {
            scm.getConnection().set("scm:git:https://github.com/firebase/firebase-android-sdk.git");
    ...
    private FirebaseStaticAnalysis initializeStaticAnalysis(Project project) {
        return new FirebaseStaticAnalysis(projectsFromProperty(project, "firebase.checks.lintProjects"));
    }
    ...
    public Set<FirebaseLibraryExtension> getLibrariesToRelease() {
        return ImmutableSet.<FirebaseLibraryExtension>builder().addAll(libraryGroupRegistrar.getLibrariesForGroup(libraryGroupName)).add(this).build();
    }
    ...
    public Set<File> getSrcDirs() {
        switch (type) {
            case ANDROID:
                return project.getExtensions().getByType(LibraryExtension.class).getSourceSets().getByName("main").getJava().getSrcDirs();
            case JAVA:
    ...
    public String toString() {
        return String.format("FirebaseLibraryExtension{name=\"%s:%s\", project=\"%s\", type=\"%s\"}", groupId.get(), artifactId.get(), getPath(), type);
    }
}
